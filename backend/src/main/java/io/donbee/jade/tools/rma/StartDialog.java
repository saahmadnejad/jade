...
import io.donbee.jade.gui.ClassSelectionDialog;
import io.donbee.jade.util.ClassFinderFilter;
...
public class StartDialog extends JDialog implements ActionListener {
	...
	private static class AgentClassFilter implements ClassFinderFilter {
		private final static String[] excluded = new String[] {
			"io.donbee.jade.domain.ams",
			"io.donbee.jade.tools.ToolNotifier",
			"io.donbee.jade.tools.logging.LogHelperAgent"
		};

		// Exclude all classes that are
		// - Not concrete (abstract or interfaces)
		// - Contained in the io.donbee.jade.core package (a part from the io.donbee.jade.core.Agent class itself)
		...
		public boolean include(Class superClazz, Class clazz) {
			...
			if (doInclude) {
				if (clazzName.startsWith("io.donbee.jade.core")) {
	...
	static {
		...
		// className = new JTextField ("io.donbee.jade.core.Agent");
		//
...
//		getContentPane().add(classNameCombo);
	...
	public void actionPerformed(ActionEvent evt) {
		if (evt.getSource() == jComboBoxClassnameCombo) {
			...
			l.add(0, "io.donbee.jade.core.Agent");
