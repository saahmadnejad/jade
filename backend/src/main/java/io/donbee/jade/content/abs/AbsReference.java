...
import io.donbee.jade.content.schema.ReferenceSchema;
import io.donbee.jade.content.schema.VariableSchema;
...
//		}
	...
	public static Class getJavaClass() {
		if (absReferenceClass == null) {
			try {
				absReferenceClass = Class.forName("io.donbee.jade.content.abs.AbsReference");
			}
			catch (Exception e) {
				// Should never happen
				e.printStackTrace();
