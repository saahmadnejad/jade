...
package io.donbee.jade.mtp.iiop;
...
public class MessageTransportProtocol implements MTP {
  ...
  private static class MTSImpl extends _MTSImplBase {
    ...
    public void message(FipaMessage aFipaMessage) {
      io.donbee.FIPA.Envelope[] envelopes = aFipaMessage.messageEnvelopes;
      ...
      for(int e = 0; e < envelopes.length; e++) {
          io.donbee.FIPA.Envelope IDLenv = envelopes[e];
    ...
    private Property unmarshalProperty(io.donbee.FIPA.Property p) {
      return new Property(p.keyword, p.value.extract_Value());
    }

    private ReceivedObject unmarshalReceivedObj(io.donbee.FIPA.ReceivedObject ro) {
  ...
  public void deliver(String addr, Envelope env, byte[] payload) throws MTPException {
	...
	//IDLencrypted[i] = (String)encrypted.get(i);
      ...
      io.donbee.FIPA.DateTime[] IDLdate = new io.donbee.FIPA.DateTime[] { marshalDateTime(env.getDate()) };
      io.donbee.FIPA.Property[][] IDLtransportBehaviour = new io.donbee.FIPA.Property[][] { };
    ...
    io.donbee.FIPA.Property[] IDLuserDefinedProperties = new io.donbee.FIPA.Property[userDefProps.size()];
      for(int i = 0; i < userDefProps.size(); i++)
	IDLuserDefinedProperties[i] = (io.donbee.FIPA.Property)userDefProps.get(i);
	...
	IDLreceived[i] = marshalReceivedObj(received[i]);
      */
      ...
      ReceivedObject received = env.getReceived();
        io.donbee.FIPA.ReceivedObject[] IDLreceived;
      if(received != null)
	IDLreceived = new io.donbee.FIPA.ReceivedObject[] { marshalReceivedObj(received) };
      else
	IDLreceived = new io.donbee.FIPA.ReceivedObject[] { };

        io.donbee.FIPA.Envelope IDLenv = new io.donbee.FIPA.Envelope(IDLto,
					       IDLfrom,
					       IDLcomments,
      ...
      FipaMessage msg = new FipaMessage(new io.donbee.FIPA.Envelope[] { IDLenv }, payload);
  ...
  public String getName() {
    return io.donbee.jade.domain.FIPANames.MTP.IIOP;
  }
  ...
  private Property marshalProperty(io.donbee.jade.domain.FIPAAgentManagement.Property p) {
  ...
  private io.donbee.FIPA.AgentID marshalAID(AID id) {
    String name = id.getName();
    String[] addresses = id.getAddressesArray();
    AID[] resolvers = id.getResolversArray();
      io.donbee.FIPA.Property[] userDefinedProperties = new io.donbee.FIPA.Property[] { };
    int numOfResolvers = resolvers.length;
      io.donbee.FIPA.AgentID result = new io.donbee.FIPA.AgentID(name, addresses, new AgentID[numOfResolvers], userDefinedProperties);
  ...
  private io.donbee.FIPA.ReceivedObject marshalReceivedObj(ReceivedObject ro) {
      io.donbee.FIPA.ReceivedObject result = new io.donbee.FIPA.ReceivedObject();
    result.by = ro.getBy();
