...
import io.donbee.jade.core.PlatformManager;
...
class CommandDispatcher implements StubHelper, ICP.Listener {
	...
	static {
		enableMultiplePlatforms = "true".equals(System.getProperty("io.donbee.jade.enable.multiple.platforms"));
	}
	...
	protected void checkRemoteExceptions(Command response) throws DispatcherException, UnreachableException {
		if (response.getCode() == Command.ERROR) {
			...
			if (exception.equals("io.donbee.jade.imtp.leap.DispatcherException")) {
				...
				if (exception.equals("io.donbee.jade.core.UnreachableException")) {
