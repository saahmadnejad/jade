...
import io.donbee.jade.imtp.leap.JICP.JICPProtocol;
import io.donbee.jade.imtp.leap.nio.BEManagementService;
...
import io.donbee.jade.security.*;
import io.donbee.jade.core.messaging.*;
...
public class BackEndContainer extends AgentContainerImpl implements BackEnd {
	...
	private static Properties adjustProperties(Properties pp) {
		...
		if (services == null) {
			services = "io.donbee.jade.core.event.NotificationService";
		}
	...
	void initMandatoryServices(List services) throws ServiceException {
		ServiceDescriptor dsc = startService("io.donbee.jade.core.management.BEAgentManagementService", false);
		dsc.setMandatory(true);
		services.add(dsc);
		dsc = startService("io.donbee.jade.core.messaging.MessagingService", false);
		dsc.setMandatory(true);
		services.add(dsc);
	}
	...
	public List removePendingMessages(MessageTemplate template, boolean notifyFailure) {
		List pendingMsg = ((io.donbee.jade.imtp.leap.FrontEndStub) myFrontEnd).removePendingMessages(template);
