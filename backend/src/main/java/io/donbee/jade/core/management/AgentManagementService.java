...
package io.donbee.jade.core.management;
...
import io.donbee.jade.core.AgentState;
...
//#J2ME_EXCLUDE_BEGIN
import io.donbee.jade.util.ObjectManager;
...
public class AgentManagementService extends BaseService {
	...
	private class CommandSourceSink implements Sink {
		...
		private void handleRequestStateChange(VerticalCommand cmd) throws IMTPException, JADESecurityException, NotFoundException, ServiceException {
			...
			if(as.equals(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.SUSPENDED)) {
				newState = Agent.AP_SUSPENDED;
			}
			else if(as.equals(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.WAITING)) {
				newState = Agent.AP_WAITING;
			}
			else if(as.equals(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.ACTIVE)) {
		...
		private void handleInformStateChanged(VerticalCommand cmd) {
			...
			if (to.equals(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.SUSPENDED)) {
				try {
					// Notify the main container through its slice
					AgentManagementSlice mainSlice = (AgentManagementSlice)getSlice(MAIN_SLICE);
			...
			else if (from.equals(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.SUSPENDED)) {
				try {
					// Notify the main container through its slice
					AgentManagementSlice mainSlice = (AgentManagementSlice)getSlice(MAIN_SLICE);
	...
	private class CommandTargetSink implements Sink {
				...
				try {
					...
					/*#CUSTOMJ2SE_INCLUDE_BEGIN
					 ...
					 //System.out.println("Replacing old agent "+name.getName());
					  ...
					  ((io.donbee.jade.core.AgentManager) impl).kill(name, principal, credentials);
					  waitUntilDead(name);
					  impl.bornAgent(name, cid, principal, ownership, false);
					  }
					  catch (Exception e) {
		...
		/*#CUSTOMJ2SE_INCLUDE_BEGIN
		 private io.donbee.jade.util.leap.List dyingAgents = new io.donbee.jade.util.leap.ArrayList();
		...
		public VerticalCommand serve(HorizontalCommand cmd) {
			...
			try {
				...
				if(cmdName.equals(AgentManagementSlice.H_CREATEAGENT)) {
					...
					AID agentID = (AID)params[0];
					gCmd.addParam(agentID);
					gCmd.addParam(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.SUSPENDED);
					gCmd.addParam("*");
					...
					AID agentID = (AID)params[0];
					gCmd.addParam(agentID);
					gCmd.addParam(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.ACTIVE);
					gCmd.addParam(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.SUSPENDED);
