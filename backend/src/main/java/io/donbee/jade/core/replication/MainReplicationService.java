...
package io.donbee.jade.core.replication;
...
import io.donbee.jade.core.PlatformManagerImpl;
...
import io.donbee.jade.core.NodeDescriptor;
import io.donbee.jade.core.NodeEventListener;
import io.donbee.jade.core.NodeFailureMonitor;

import io.donbee.jade.core.AgentContainer;
import io.donbee.jade.core.MainContainerImpl;
...
import io.donbee.jade.util.InputQueue;
	...
	private class CommandOutgoingFilter extends Filter {
		...
		public boolean accept(VerticalCommand cmd) {
			...
			try {
				...
				if (name.equals(io.donbee.jade.core.management.AgentManagementSlice.ADD_TOOL)) {
					handleNewTool(cmd);
				} 
				else if (name.equals(io.donbee.jade.core.management.AgentManagementSlice.REMOVE_TOOL)) {
					handleDeadTool(cmd);
				} 
				//#PJAVA_EXCLUDE_BEGIN
				else if (name.equals(io.donbee.jade.core.nodeMonitoring.UDPNodeMonitoringService.ORPHAN_NODE)) {
	...
	private class CommandIncomingFilter extends Filter {
		...
		public void postProcess(VerticalCommand cmd) {
			try {
				...
				if (name.equals(io.donbee.jade.core.management.AgentManagementSlice.INFORM_CREATED)) {
					handleInformCreated(cmd);
				} else if (name.equals(io.donbee.jade.core.management.AgentManagementSlice.INFORM_KILLED)) {
					handleInformKilled(cmd);
				} else if (name.equals(io.donbee.jade.core.management.AgentManagementSlice.INFORM_STATE_CHANGED)) {
					handleInformStateChanged(cmd);
				} else if (name.equals(io.donbee.jade.core.messaging.MessagingSlice.NEW_MTP)) {
					handleNewMTP(cmd);
				} else if (name.equals(io.donbee.jade.core.messaging.MessagingSlice.DEAD_MTP)) {
					handleDeadMTP(cmd);
				} 
			} 
			catch (Throwable t) {
		...
		private void handleInformStateChanged(VerticalCommand cmd) throws IMTPException, NotFoundException, ServiceException {
			...
			if (newState.equals(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.SUSPENDED)) {
				GenericCommand hCmd = new GenericCommand(MainReplicationSlice.H_SUSPENDEDAGENT, MainReplicationSlice.NAME, null);
				hCmd.addParam(agentID);

				broadcastToReplicas(hCmd, EXCLUDE_MYSELF);
			} else if (newState.equals(io.donbee.jade.domain.FIPAAgentManagement.AMSAgentDescription.ACTIVE)) {
