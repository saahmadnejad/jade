...
import io.donbee.jade.util.leap.ArrayList;
import io.donbee.jade.util.leap.Properties;
import io.donbee.jade.util.Logger;
import io.donbee.jade.lang.acl.ACLMessage;
import io.donbee.jade.core.behaviours.Behaviour;
import io.donbee.jade.core.messaging.GenericMessage;
import io.donbee.jade.core.management.AgentManagementSlice;

//#MIDP_EXCLUDE_BEGIN
import io.donbee.jade.domain.AMSEventQueueFeeder;
//#MIDP_EXCLUDE_END
import io.donbee.jade.domain.FIPANames;
import io.donbee.jade.domain.FIPAAgentManagement.InternalError;
import io.donbee.jade.domain.JADEAgentManagement.JADEManagementOntology;
import io.donbee.jade.mtp.MTPDescriptor;
import io.donbee.jade.mtp.TransportAddress;
import io.donbee.jade.security.JADESecurityException;
import io.donbee.jade.security.CredentialsHelper;
import io.donbee.jade.security.JADEPrincipal;
import io.donbee.jade.security.Credentials;
...
class AgentContainerImpl implements AgentContainer, AgentToolkit {
	...
	public static final String MONITOR_AGENT_CLASS = "io.donbee.jade.core.ContainerMonitorAgent";
	...
	/////////////////////////////////////////////////
	io.donbee.jade.wrapper.AgentContainer getContainerController() {
		return getContainerController(myNodeDescriptor.getOwnerPrincipal(), myNodeDescriptor.getOwnerCredentials());
	}

	public io.donbee.jade.wrapper.AgentContainer getContainerController(JADEPrincipal principal, Credentials credentials) {
		return new io.donbee.jade.wrapper.AgentContainer(getContainerProxy(principal, credentials), this, getPlatformID());
	}
	 ...
	 */
	private io.donbee.jade.wrapper.ContainerProxy getContainerProxy(final JADEPrincipal principal, final Credentials credentials) {
		return new io.donbee.jade.wrapper.ContainerProxy() {
			...
			public MTPDescriptor installMTP(String address, String className) throws Throwable {
				// Do as if it was a remote call from the main to allow
				// security checks to take place if needed
				io.donbee.jade.core.messaging.MessagingSlice target = (io.donbee.jade.core.messaging.MessagingSlice) getProxyToLocalSlice(io.donbee.jade.core.messaging.MessagingSlice.NAME);
				// FIXME: set Principal and Credentials
			...
			public void suspendAgent(AID id) throws Throwable {
				// Do as if it was a remote call from the main to allow
				// security checks to take place if needed
				io.donbee.jade.core.management.AgentManagementSlice target = (io.donbee.jade.core.management.AgentManagementSlice) getProxyToLocalSlice(io.donbee.jade.core.management.AgentManagementSlice.NAME);
				// FIXME: set Principal and Credentials
			...
			public void moveAgent(AID id, Location where) throws Throwable {
				// Do as if it was a remote call from the main to allow
				// security checks to take place if needed
				io.donbee.jade.core.mobility.AgentMobilitySlice target = (io.donbee.jade.core.mobility.AgentMobilitySlice) getProxyToLocalSlice(io.donbee.jade.core.mobility.AgentMobilitySlice.NAME);
				// FIXME: set Principal and Credentials
	...
	throws NameClashException, IMTPException, NotFoundException, JADESecurityException {
		...
		GenericCommand cmd = new GenericCommand(io.donbee.jade.core.management.AgentManagementSlice.INFORM_CREATED, io.donbee.jade.core.management.AgentManagementSlice.NAME, null);
	...
	void initMandatoryServices(List services) throws ServiceException {
		ServiceDescriptor dsc = startService("io.donbee.jade.core.management.AgentManagementService", false);
		dsc.setMandatory(true);
		services.add(dsc);
		
		//#MIDP_EXCLUDE_BEGIN
		String messagingServiceClass = myProfile.getParameter("messaging-service-class", "io.donbee.jade.core.messaging.MessagingService");
		dsc = startService(messagingServiceClass, false);
		//#MIDP_EXCLUDE_END
		/*#MIDP_INCLUDE_BEGIN
		 dsc = startService("io.donbee.jade.core.messaging.LightMessagingService", false);
		 #MIDP_INCLUDE_END*/
		dsc.setMandatory(true);
		services.add(dsc);
		
		//#J2ME_EXCLUDE_BEGIN
		dsc = startService("io.donbee.jade.core.resource.ResourceManagementService", false);
		dsc.setMandatory(true);
		services.add(dsc);
		//#J2ME_EXCLUDE_END
	}
	...
	void bootAllServices(List services) throws ServiceException {
		...
		while (it.hasNext()) {
			...
			try {
				...
				if (dsc.getName().equals("io.donbee.jade.core.security.Security")) {
			...
			while(agentSpecifiers.hasNext()) {
				...
				if (s.getName() != null) {
					...
					try {
						...
						//#MIDP_EXCLUDE_END
						/*#MIDP_INCLUDE_BEGIN
						 String serviceName = io.donbee.jade.core.management.AgentManagementSlice.NAME;
						 Service svc = myServiceFinder.findService(serviceName);
						 io.donbee.jade.core.management.AgentManagementSlice target = (io.donbee.jade.core.management.AgentManagementSlice) myIMTPManager.createSliceProxy(serviceName, svc.getHorizontalInterface(), myIMTPManager.getLocalNode());
						 GenericCommand dummyCmd = new GenericCommand(null, null, null);
	...
	public void handleSend(ACLMessage msg, AID sender, boolean needClone) {
		...
		while (it.hasNext()) {
			...
			GenericCommand cmd = new GenericCommand(io.donbee.jade.core.messaging.MessagingSlice.SEND_MESSAGE, io.donbee.jade.core.messaging.MessagingSlice.NAME, null);
			cmd.addParam(sender);
			...
			if (ret != null) {
				if (ret instanceof Throwable) {
					...
					cmd = new GenericCommand(io.donbee.jade.core.messaging.MessagingSlice.NOTIFY_FAILURE, io.donbee.jade.core.messaging.MessagingSlice.NAME, null);
					cmd.addParam(gmsg);
					cmd.addParam(receiver);
	...
	public void handlePosted(AID agentID, ACLMessage msg) {
		GenericCommand cmd = new GenericCommand(io.donbee.jade.core.event.NotificationSlice.NOTIFY_POSTED, io.donbee.jade.core.event.NotificationSlice.NAME, null);
		cmd.addParam(msg);
		cmd.addParam(agentID);
	...
	public void handleReceived(AID agentID, ACLMessage msg) {
		GenericCommand cmd = new GenericCommand(io.donbee.jade.core.event.NotificationSlice.NOTIFY_RECEIVED, io.donbee.jade.core.event.NotificationSlice.NAME, null);
		cmd.addParam(msg);
		cmd.addParam(agentID);
	...
	public void handleBehaviourAdded(AID agentID, Behaviour b) {
		GenericCommand cmd = new GenericCommand(io.donbee.jade.core.event.NotificationSlice.NOTIFY_BEHAVIOUR_ADDED, io.donbee.jade.core.event.NotificationSlice.NAME, null);
		cmd.addParam(agentID);
		cmd.addParam(b);
		// No security check is meaningful on this action --> don't even set the Credentials
	...
	public void handleBehaviourRemoved(AID agentID, Behaviour b) {
		GenericCommand cmd = new GenericCommand(io.donbee.jade.core.event.NotificationSlice.NOTIFY_BEHAVIOUR_REMOVED, io.donbee.jade.core.event.NotificationSlice.NAME, null);
		cmd.addParam(agentID);
		cmd.addParam(b);
		// No security check is meaningful on this action --> don't even set the Credentials
	...
	public void handleChangeBehaviourState(AID agentID, Behaviour b, String from, String to) {
		GenericCommand cmd = new GenericCommand(io.donbee.jade.core.event.NotificationSlice.NOTIFY_CHANGED_BEHAVIOUR_STATE, io.donbee.jade.core.event.NotificationSlice.NAME, null);
		cmd.addParam(agentID);
		cmd.addParam(b);
		cmd.addParam(from);
		cmd.addParam(to);
	...
	public void handleChangedAgentState(AID agentID, int oldState, int newState) {
		...
		GenericCommand cmd = new GenericCommand(io.donbee.jade.core.management.AgentManagementSlice.INFORM_STATE_CHANGED, io.donbee.jade.core.management.AgentManagementSlice.NAME, null);
		cmd.addParam(agentID);
		cmd.addParam(from);
		cmd.addParam(to);
		// No security check is meaningful on this action --> don't even set the Credentials
	...
	public void handleEnd(AID agentID) {
		GenericCommand cmd = new GenericCommand(io.donbee.jade.core.management.AgentManagementSlice.INFORM_KILLED, io.donbee.jade.core.management.AgentManagementSlice.NAME, null);
		cmd.addParam(agentID);
	...
	public void setPlatformAddresses(AID id) {
		GenericCommand cmd = new GenericCommand(io.donbee.jade.core.messaging.MessagingSlice.SET_PLATFORM_ADDRESSES, io.donbee.jade.core.messaging.MessagingSlice.NAME, null);
		cmd.addParam(id);
		// No security check is meaningful on this action --> don't even set the Credentials
	...
	private void initCredentials(Command cmd, AID id) {
		...
		if (securityOn) {
			...
			if (agent != null) {
				try {
					CredentialsHelper ch = (CredentialsHelper) agent.getHelper("io.donbee.jade.core.security.Security");
