...
package ir.donbee.jade.mtp.iiop;
...
import ir.donbee.FIPA.*; // OMG IDL Stubs

import ir.donbee.jade.core.AID;
import ir.donbee.jade.core.Profile;

import ir.donbee.jade.mtp.InChannel;
import ir.donbee.jade.mtp.OutChannel;
import ir.donbee.jade.mtp.MTP;
import ir.donbee.jade.mtp.MTPException;
import ir.donbee.jade.mtp.TransportAddress;
import ir.donbee.jade.domain.FIPAAgentManagement.Envelope;
import ir.donbee.jade.domain.FIPAAgentManagement.ReceivedObject;
import ir.donbee.jade.domain.FIPAAgentManagement.Property;
...
/**
   Implementation of <code><b>mts.mtp.iiop.std</b></code>
...
public class MessageTransportProtocol implements MTP {
  ...
  private static class MTSImpl extends _MTSImplBase {
    ...
    public void message(FipaMessage aFipaMessage) {
      ir.donbee.FIPA.Envelope[] envelopes = aFipaMessage.messageEnvelopes;
      ...
      for(int e = 0; e < envelopes.length; e++) {
          ir.donbee.FIPA.Envelope IDLenv = envelopes[e];
    ...
    private Property unmarshalProperty(ir.donbee.FIPA.Property p) {
      return new Property(p.keyword, p.value.extract_Value());
    }

    private ReceivedObject unmarshalReceivedObj(ir.donbee.FIPA.ReceivedObject ro) {
  ...
  public void deliver(String addr, Envelope env, byte[] payload) throws MTPException {
	...
	IDLto[i] = (AgentID)to.get(i);
      ...
      AgentID[] IDLfrom = new AgentID[] { marshalAID(from) };
	...
	//IDLencrypted[i] = (String)encrypted.get(i);
      ...
      ir.donbee.FIPA.DateTime[] IDLdate = new ir.donbee.FIPA.DateTime[] { marshalDateTime(env.getDate()) };
      ir.donbee.FIPA.Property[][] IDLtransportBehaviour = new ir.donbee.FIPA.Property[][] { };
    ...
    ir.donbee.FIPA.Property[] IDLuserDefinedProperties = new ir.donbee.FIPA.Property[userDefProps.size()];
      for(int i = 0; i < userDefProps.size(); i++)
	IDLuserDefinedProperties[i] = (ir.donbee.FIPA.Property)userDefProps.get(i);
      ...
      ReceivedObject[] IDLreceived = new ReceivedObject[received.length];
	...
	IDLreceived[i] = marshalReceivedObj(received[i]);
      */
      ...
      ReceivedObject received = env.getReceived();
        ir.donbee.FIPA.ReceivedObject[] IDLreceived;
      if(received != null)
	IDLreceived = new ir.donbee.FIPA.ReceivedObject[] { marshalReceivedObj(received) };
      else
	IDLreceived = new ir.donbee.FIPA.ReceivedObject[] { };

        ir.donbee.FIPA.Envelope IDLenv = new ir.donbee.FIPA.Envelope(IDLto,
					       IDLfrom,
					       IDLcomments,
      ...
      FipaMessage msg = new FipaMessage(new ir.donbee.FIPA.Envelope[] { IDLenv }, payload);
  ...
  public String getName() {
    return ir.donbee.jade.domain.FIPANames.MTP.IIOP;
  }
  ...
  private Property marshalProperty(ir.donbee.jade.domain.FIPAAgentManagement.Property p) {
  ...
  private ir.donbee.FIPA.AgentID marshalAID(AID id) {
    String name = id.getName();
    String[] addresses = id.getAddressesArray();
    AID[] resolvers = id.getResolversArray();
      ir.donbee.FIPA.Property[] userDefinedProperties = new ir.donbee.FIPA.Property[] { };
    int numOfResolvers = resolvers.length;
      ir.donbee.FIPA.AgentID result = new ir.donbee.FIPA.AgentID(name, addresses, new AgentID[numOfResolvers], userDefinedProperties);
  ...
  private DateTime marshalDateTime(Date d) {
    ...
    DateTime result = new DateTime(year,
					     month,
					     day,
					     hour,
					     minutes,
  ...
  private ir.donbee.FIPA.ReceivedObject marshalReceivedObj(ReceivedObject ro) {
      ir.donbee.FIPA.ReceivedObject result = new ir.donbee.FIPA.ReceivedObject();
    result.by = ro.getBy();
