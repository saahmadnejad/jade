 ...
 * @see ir.donbee.jade.core.NodeEventListener
 */
public abstract class NodeFailureMonitor {
	...
	public static NodeFailureMonitor getDefaultFailureMonitor() {
		try {
			return (NodeFailureMonitor) Class.forName("ir.donbee.jade.core.nodeMonitoring.BlockingNodeFailureMonitor").newInstance();
		}
		catch (Throwable t) {
			// Should never happen
			t.printStackTrace();
