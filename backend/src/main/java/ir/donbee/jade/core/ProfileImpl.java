...
public class ProfileImpl extends Profile {
	...
	public ProfileImpl(boolean isMain) {
		...
		//#ALL_EXCLUDE_BEGIN
		props = new ir.donbee.jade.util.ExtendedProperties();
		//#ALL_EXCLUDE_END
	...
	void init() {
		...
		try {
			...
			if (isGui != null && CaseInsensitiveString.equalsIgnoreCase(isGui, "true")) {
				...
				s.setName("rma");
				s.setClassName("ir.donbee.jade.tools.rma.rma");
				agents.add(0, s);
		...
		if(isMain() && (!getBooleanProperty("nomtp", false)) && (props.getProperty(MTPS) == null)) {
			Specifier s = new Specifier();
			s.setClassName("ir.donbee.jade.mtp.http.MessageTransportProtocol"); 
	...
	private void createIMTPManager() throws ProfileException {
		...
		if (imtp.equals(LEAP_IMTP)) {
			imtpClass = "ir.donbee.jade.imtp.leap.LEAPIMTPManager";
		}
		else if (imtp.equals(RMI_IMTP)) {
			imtpClass = "ir.donbee.jade.imtp.rmi.RMIIMTPManager";
		}
		try {
	...
	private void createResourceManager() throws ProfileException {
		...
		String className = getParameter(RESOURCE, "ir.donbee.jade.core.FullResourceManager");
		//#J2ME_EXCLUDE_END
		/*#J2ME_INCLUDE_BEGIN
		 String className = getParameter(RESOURCE, "ir.donbee.jade.core.LightResourceManager");
		 #J2ME_INCLUDE_END*/
