...
import ir.donbee.jade.gui.ClassSelectionDialog;
import ir.donbee.jade.util.ClassFinderFilter;
...
public class StartDialog extends JDialog implements ActionListener {
	...
	private static class AgentClassFilter implements ClassFinderFilter {
		private final static String[] excluded = new String[] {
			"ir.donbee.jade.domain.ams",
			"ir.donbee.jade.tools.ToolNotifier",
			"ir.donbee.jade.tools.logging.LogHelperAgent"
		};

		// Exclude all classes that are
		// - Not concrete (abstract or interfaces)
		// - Contained in the ir.donbee.jade.core package (a part from the ir.donbee.jade.core.Agent class itself)
		...
		public boolean include(Class superClazz, Class clazz) {
			...
			if (doInclude) {
				if (clazzName.startsWith("ir.donbee.jade.core")) {
	...
	static {
		...
		// className = new JTextField ("ir.donbee.jade.core.Agent");
		//
...
//		getContentPane().add(classNameCombo);
	...
	public void actionPerformed(ActionEvent evt) {
		if (evt.getSource() == jComboBoxClassnameCombo) {
			...
			l.add(0, "ir.donbee.jade.core.Agent");
