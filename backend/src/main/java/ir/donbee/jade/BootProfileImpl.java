...
import ir.donbee.jade.core.ProfileImpl;
...
public class BootProfileImpl extends ProfileImpl {
	...
	public void setArgProperties(ExtendedProperties source) {
		...
		if (flag) {
			...
			if (value != null) {
				value = "RMA:ir.donbee.jade.tools.rma.rma " + value;  // put before other agents
			} else {
				value = "RMA:ir.donbee.jade.tools.rma.rma";  // only one
			}
		}
		...
		// Note All the ir.donbee.jade.util.leap.ArrayList structures will only print their type unless a
	 ...
	 * <p>
	 * For instance
	 * <code>ir.donbee.jade.mtp.iiop(50);http.mtp.http(8080)</code> is a valid
	 * string, while  <code>ir.donbee.jade.mtp.iiop(50 80);http.mtp.http(8080)</code>
	 * is not valid
