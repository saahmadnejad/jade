 ...
 * **************************************************************
 */
package ir.donbee.jade.content.lang.sl;

import ir.donbee.jade.content.*;
import ir.donbee.jade.content.onto.*;
import ir.donbee.jade.content.schema.*;
 ...
 * see ir.donbee.jade.content.Ontology
 * @author Giovanni Caire - TILAB
 */
class SL2Ontology extends SL1Ontology implements SL2Vocabulary {
	// NAME
	public static final String ONTOLOGY_NAME = ir.donbee.jade.domain.FIPANames.Ontology.SL2_ONTOLOGY;;
	...
	protected SL2Ontology(String name, Ontology base, Introspector intro) {
		...
		try {
			...
			add(new PredicateSchema(FOREACH), AbsPredicate.getJavaClass());
			...
			ps = (PredicateSchema) getSchema(FOREACH);
			ps.add(FOREACH_WHAT, (VariableSchema) VariableSchema.getBaseSchema());
			ps.add(FOREACH_WHERE, (PredicateSchema) PredicateSchema.getBaseSchema());
			ps.add(FOREACH_CONDITION, (PredicateSchema) PredicateSchema.getBaseSchema());
	...
	boolean isConditionedQuantifier(String symbol) {
		return CaseInsensitiveString.equalsIgnoreCase(FOREACH, symbol);
	}
