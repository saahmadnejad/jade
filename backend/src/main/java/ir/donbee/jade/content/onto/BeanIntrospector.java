...
import ir.donbee.jade.content.Term;
...
import ir.donbee.jade.content.abs.AbsPrimitive;
import ir.donbee.jade.content.abs.AbsTerm;
...
class BeanIntrospector implements Introspector {
	...
	public AbsAggregate externalizeAggregate(String slotName, Object slotValue, ObjectSchema schema, Ontology referenceOnto) throws OntologyException {
		...
		if (valueClass.isArray() && valueClass != byte[].class) {
			...
			if (slotValue instanceof java.util.Collection) {
			...
			} else if (slotValue instanceof ir.donbee.jade.util.leap.Collection) {
				// In the case of array and slot not typized --> throw exception
				// (Only java collection are permitted)
				if (!slotTypized) {
				...
				iter = ((ir.donbee.jade.util.leap.Collection)slotValue).iterator();
				if (slotValue instanceof ir.donbee.jade.util.leap.List) {
					aggregateType = BasicOntology.SEQUENCE;
				} else {
					aggregateType = BasicOntology.SET;
				}
	...
	public Object internalizeAggregate(String slotName, AbsAggregate absAggregate, ObjectSchema schema, Ontology referenceOnto) throws OntologyException {
		...
		if (aggregateClass == null || 
			...
			aggregateClass == ir.donbee.jade.util.leap.Collection.class ||
		...
		Object result;
		ir.donbee.jade.util.leap.Iterator iterator = absAggregate.iterator();
		try {
			...
			} else if (ir.donbee.jade.util.leap.Collection.class.isAssignableFrom(aggregateClass)) {
				ir.donbee.jade.util.leap.Collection jadeCollection = AggregateHelper.createConcreteJadeCollection(aggregateClass);
