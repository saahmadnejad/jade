...
public class BCReflectiveIntrospector extends ReflectiveIntrospector {
	...
	private Class checkGetAndSet2(String name, Class c) throws OntologyException {
		...
		// must be a ir.donbee.jade.util.leap.Iterator or a super-class/interface of it -->
		// OK if it is a java.util.Iterator.
		if (!(getReturnType(getMethod)).isAssignableFrom(ir.donbee.jade.util.leap.Iterator.class))
			throw new OntologyException("Wrong class: method " +  getMethod.getName() + "() must return a ir.donbee.jade.util.leap.Iterator." + getReturnType(getMethod).toString());
