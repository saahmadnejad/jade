...
import io.donbee.jade.core.sam.CounterValueProvider;
...
import io.donbee.jade.util.ThreadDumpManager;
import io.donbee.jade.util.leap.Properties;
import io.donbee.jade.imtp.leap.TransportProtocol;
import io.donbee.jade.imtp.leap.FrontEndStub;
...
server in the form of <code>io.donbee.jade.imtp.leap.JICP.ProtocolManager<code> 
class
</li>
<li>
...
public class BEManagementService extends BaseService {
	...
	private void handleAcceptOption(Profile p) throws ProfileException {
		...
		if (acceptOption != null) {
			...
			io.donbee.jade.util.leap.List acceptedProtocols = p.getSpecifiers(ACCEPT);
				...
				case JICPProtocol.CREATE_MEDIATOR_TYPE: {
					...
					if (mediator == null) {
						...
						if (id != null) {
							...
							p.setProperty(io.donbee.jade.core.BackEndContainer.RESYNCH, "true");
						} else {
