...
public class AggregateHelper {
	...
	public static ObjectSchema getSchema(Class clazz, TermSchema elementSchema) {
		...
		if (java.util.List.class.isAssignableFrom(clazz) ||
			io.donbee.jade.util.leap.List.class.isAssignableFrom(clazz) ||
		...
		// Set type
			...
			io.donbee.jade.util.leap.Set.class.isAssignableFrom(clazz)) {
		...
		if (srcValue != null) {
			...
			if (srcClass != destClass) {
				...
				if (destClass.isArray()) {
					...
					else if (io.donbee.jade.util.leap.Collection.class.isAssignableFrom(srcClass)) {
						io.donbee.jade.util.leap.Collection jadeCollection = (io.donbee.jade.util.leap.Collection)srcValue;
				...
				else if (io.donbee.jade.util.leap.Collection.class.isAssignableFrom(destClass)) {

					// Source is an array
					if (srcClass.isArray()) {
						io.donbee.jade.util.leap.Collection jadeCollection = createConcreteJadeCollection(destClass);
						int size = Array.getLength(srcValue);
						for (int index=0; index<size; index++) {
	...
	static io.donbee.jade.util.leap.Collection createConcreteJadeCollection(Class clazz) throws InstantiationException, IllegalAccessException {
		int modifiers = clazz.getModifiers();
		io.donbee.jade.util.leap.Collection result = null;
		if ((modifiers & ACC_ABSTRACT) == 0 && (modifiers & ACC_INTERFACE) == 0) {
			// class is concrete, we can instantiate it directly
			result = (io.donbee.jade.util.leap.Collection) clazz.newInstance();
		} else {
			// class is either abstract or an interface, we have to somehow choose a concrete collection :-(
			if (io.donbee.jade.util.leap.List.class.isAssignableFrom(clazz)) {
				result = new io.donbee.jade.util.leap.ArrayList();
			} else if (io.donbee.jade.util.leap.Set.class.isAssignableFrom(clazz)) {
				result = new io.donbee.jade.util.leap.HashSet();
			}
		}
		return result;
	}
