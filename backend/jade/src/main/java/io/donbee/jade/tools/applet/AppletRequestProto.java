...
package io.donbee.jade.tools.applet;
...
import io.donbee.jade.lang.acl.ACLParser;
import io.donbee.jade.lang.acl.ParseException;

import io.donbee.jade.content.abs.*;
...
abstract class AppletRequestProto
{
   ...
   */
     ...
     **/
    static String encode(Action act, SLCodec c, Ontology o) throws io.donbee.jade.domain.FIPAException {
    // Write the action in the :content slot of the request
        try {
            return c.encode((AbsContentElement)o.fromObject(act));
        } catch (io.donbee.jade.content.onto.OntologyException oe) {
            throw new io.donbee.jade.domain.FIPAException(oe.getMessage());
        } catch (Codec.CodecException el) {
            el.printStackTrace();
            throw new io.donbee.jade.domain.FIPAException(el.getMessage());
        }
    }
   ...
   */
  static Result extractContent(String content, SLCodec c, Ontology o) throws io.donbee.jade.domain.FIPAException {
    try {
      AbsContentElement tuple = c.decode(o, content);
      return (Result)o.toObject(tuple);
    } catch (Codec.CodecException e1) {
      e1.printStackTrace();
      throw new io.donbee.jade.domain.FIPAException(e1.getMessage());
    } catch (io.donbee.jade.content.onto.OntologyException e2) {
      e2.printStackTrace();
      throw new io.donbee.jade.domain.FIPAException(e2.getMessage());
    }
  }
}
