...
import io.donbee.jade.util.leap.Collection;
...
public class BCReflectiveIntrospector extends ReflectiveIntrospector {
	...
	private Class checkGetAndSet2(String name, Class c) throws OntologyException {
		...
		// must be a io.donbee.jade.util.leap.Iterator or a super-class/interface of it -->
		// OK if it is a java.util.Iterator.
		if (!(getReturnType(getMethod)).isAssignableFrom(io.donbee.jade.util.leap.Iterator.class))
			throw new OntologyException("Wrong class: method " +  getMethod.getName() + "() must return a io.donbee.jade.util.leap.Iterator." + getReturnType(getMethod).toString());
