...
public class BootProfileImpl extends ProfileImpl {
	...
	public void setArgProperties(ExtendedProperties source) {
		...
		if (flag) {
			...
			if (value != null) {
				value = "RMA:io.donbee.jade.tools.rma.rma " + value;  // put before other agents
			} else {
				value = "RMA:io.donbee.jade.tools.rma.rma";  // only one
			}
		}
		...
		// Note All the io.donbee.jade.util.leap.ArrayList structures will only print their type unless a
	 ...
	 * <p>
	 * For instance
	 * <code>io.donbee.jade.mtp.iiop(50);http.mtp.http(8080)</code> is a valid
	 * string, while  <code>io.donbee.jade.mtp.iiop(50 80);http.mtp.http(8080)</code>
	 * is not valid
