...
import ir.donbee.jade.imtp.leap.nio.BEManagementService;
...
import ir.donbee.jade.core.messaging.*;
...
public class BackEndContainer extends AgentContainerImpl implements BackEnd {
	...
	private static Properties adjustProperties(Properties pp) {
		...
		if (services == null) {
			services = "ir.donbee.jade.core.event.NotificationService";
		}
	...
	void initMandatoryServices(List services) throws ServiceException {
		ServiceDescriptor dsc = startService("ir.donbee.jade.core.management.BEAgentManagementService", false);
		dsc.setMandatory(true);
		services.add(dsc);
		dsc = startService("ir.donbee.jade.core.messaging.MessagingService", false);
		dsc.setMandatory(true);
		services.add(dsc);
	}
	...
	public List removePendingMessages(MessageTemplate template, boolean notifyFailure) {
		List pendingMsg = ((ir.donbee.jade.imtp.leap.FrontEndStub) myFrontEnd).removePendingMessages(template);
