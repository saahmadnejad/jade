	...
	private void manageProtoOption(Properties pp) {
		...
		if (proto != null) {
			...
			if (CaseInsensitiveString.equalsIgnoreCase(MicroRuntime.SOCKET_PROTOCOL, proto)) {
				pp.setProperty(MicroRuntime.CONN_MGR_CLASS_KEY, "ir.donbee.jade.imtp.leap.JICP.FrontEndDispatcher");
			}
			else if (CaseInsensitiveString.equalsIgnoreCase(MicroRuntime.SSL_PROTOCOL, proto)) {
				pp.setProperty(MicroRuntime.CONN_MGR_CLASS_KEY, "ir.donbee.jade.imtp.leap.JICP.FrontEndSDispatcher");
			}
			else if (CaseInsensitiveString.equalsIgnoreCase(MicroRuntime.HTTP_PROTOCOL, proto)) {
				pp.setProperty(MicroRuntime.CONN_MGR_CLASS_KEY, "ir.donbee.jade.imtp.leap.http.HTTPFEDispatcher");
				pp.setProperty(JICPProtocol.MEDIATOR_CLASS_KEY, "ir.donbee.jade.imtp.leap.nio.NIOHTTPBEDispatcher");
			}
			else if (CaseInsensitiveString.equalsIgnoreCase(MicroRuntime.HTTPS_PROTOCOL, proto)) {
				pp.setProperty(MicroRuntime.CONN_MGR_CLASS_KEY, "ir.donbee.jade.imtp.leap.http.HTTPFESDispatcher");
				pp.setProperty(JICPProtocol.MEDIATOR_CLASS_KEY, "ir.donbee.jade.imtp.leap.nio.NIOHTTPBEDispatcher");
			}
		}
	}
