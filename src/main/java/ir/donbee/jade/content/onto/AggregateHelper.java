...
import ir.donbee.jade.content.schema.TermSchema;
...
public class AggregateHelper {
	...
	public static ObjectSchema getSchema(Class clazz, TermSchema elementSchema) {
		...
		if (java.util.List.class.isAssignableFrom(clazz) ||
			ir.donbee.jade.util.leap.List.class.isAssignableFrom(clazz) ||
		...
		// Set type
			...
			ir.donbee.jade.util.leap.Set.class.isAssignableFrom(clazz)) {
		...
		if (srcValue != null) {
			...
			if (srcClass != destClass) {
				...
				if (destClass.isArray()) {
					...
					else if (ir.donbee.jade.util.leap.Collection.class.isAssignableFrom(srcClass)) {
						ir.donbee.jade.util.leap.Collection jadeCollection = (ir.donbee.jade.util.leap.Collection)srcValue;
				...
				else if (ir.donbee.jade.util.leap.Collection.class.isAssignableFrom(destClass)) {

					// Source is an array
					if (srcClass.isArray()) {
						ir.donbee.jade.util.leap.Collection jadeCollection = createConcreteJadeCollection(destClass);
						int size = Array.getLength(srcValue);
						for (int index=0; index<size; index++) {
	...
	static ir.donbee.jade.util.leap.Collection createConcreteJadeCollection(Class clazz) throws InstantiationException, IllegalAccessException {
		int modifiers = clazz.getModifiers();
		ir.donbee.jade.util.leap.Collection result = null;
		if ((modifiers & ACC_ABSTRACT) == 0 && (modifiers & ACC_INTERFACE) == 0) {
			// class is concrete, we can instantiate it directly
			result = (ir.donbee.jade.util.leap.Collection) clazz.newInstance();
		} else {
			// class is either abstract or an interface, we have to somehow choose a concrete collection :-(
			if (ir.donbee.jade.util.leap.List.class.isAssignableFrom(clazz)) {
				result = new ir.donbee.jade.util.leap.ArrayList();
			} else if (ir.donbee.jade.util.leap.Set.class.isAssignableFrom(clazz)) {
				result = new ir.donbee.jade.util.leap.HashSet();
			}
		}
		return result;
	}
