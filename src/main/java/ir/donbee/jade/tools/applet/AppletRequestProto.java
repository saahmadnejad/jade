...
package ir.donbee.jade.tools.applet;
...
import ir.donbee.jade.lang.acl.ACLParser;
import ir.donbee.jade.lang.acl.ParseException;

import ir.donbee.jade.content.abs.*;
...
abstract class AppletRequestProto
{
   ...
   */
     ...
     **/
    static String encode(Action act, SLCodec c, Ontology o) throws ir.donbee.jade.domain.FIPAException {
    // Write the action in the :content slot of the request
        try {
            return c.encode((AbsContentElement)o.fromObject(act));
        } catch (ir.donbee.jade.content.onto.OntologyException oe) {
            throw new ir.donbee.jade.domain.FIPAException(oe.getMessage());
        } catch (Codec.CodecException el) {
            el.printStackTrace();
            throw new ir.donbee.jade.domain.FIPAException(el.getMessage());
        }
    }
   ...
   */
  static Result extractContent(String content, SLCodec c, Ontology o) throws ir.donbee.jade.domain.FIPAException {
    try {
      AbsContentElement tuple = c.decode(o, content);
      return (Result)o.toObject(tuple);
    } catch (Codec.CodecException e1) {
      e1.printStackTrace();
      throw new ir.donbee.jade.domain.FIPAException(e1.getMessage());
    } catch (ir.donbee.jade.content.onto.OntologyException e2) {
      e2.printStackTrace();
      throw new ir.donbee.jade.domain.FIPAException(e2.getMessage());
    }
  }
}
