   ...
   try 
   {
     ...
     if((typeCanv == false)) 
	 {
       ...
       for(int num = 0; num < xCanvDim; num++) {
	     ...
	     //#DOTNET_EXCLUDE_BEGIN
         int x =  ir.donbee.jade.tools.sniffer.Agent.yRet/2+num*80;
         g.setColor(new Color(0,100,50));
	     //#DOTNET_EXCLUDE_END
	     /*#DOTNET_INCLUDE_BEGIN
		 int x = (int) ir.donbee.jade.tools.sniffer.Agent.yRet/2+num*80;
  ...
  #DOTNET_INCLUDE_END*/
   ...
   int numberToShow=5;
    ...
    if( ((mess = selMessage(evt)) != null) && (typeCanv == false)) {
	   ...
	   //#DOTNET_EXCLUDE_END
	   /*#DOTNET_INCLUDE_BEGIN
  	   messageToDisplay = ir.donbee.jade.lang.acl.ACLMessage.getPerformative(
	   #DOTNET_INCLUDE_END*/
